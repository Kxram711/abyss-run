...
local WELL_COLOR = Color3.fromRGB(96, 240, 255) -- Lumen Well glow (GDD §4.2)
local WELL_MARGIN = 4 -- studs from room walls when placing a Well
...
function FloorService:KnitStart()
    ...
    self.LumenService = Knit.GetService("LumenService") -- for Well registration (wells = touch recharge, §4.2)
...
function FloorService:_BuildFloor(floor: number)
    ...
    -- One Lumen Well per floor, in a mid-map room (GDD §4.3). Placed with the
    -- same floor-seeded RNG so a run seed's layout is fully deterministic.
    self:_PlaceLumenWell(floorFolder, rng, middle, floor)
end

--- Places the floor's single Lumen Well (primitive glowing part) at a
--- deterministic interior point of a randomly chosen mid-map room, and
--- registers it with LumenService so touching it recharges the pool (+40,
--- v1 target). LumenService:DepleteWell dims the "CoreLight" child on use.
function FloorService:_PlaceLumenWell(floorFolder: Folder, rng: any, middle: { RoomTemplates.RoomTemplate }, floor: number)
    if #middle == 0 then
        return
    end
    local template = middle[rng:NextInt(1, #middle)]
    local roomModel = floorFolder:FindFirstChild("Room_" .. template.Name)
    if roomModel == nil then
        return
    end

    local x = WELL_MARGIN + rng:Next() * (template.Size.X - 2 * WELL_MARGIN)
    local z = WELL_MARGIN + rng:Next() * (template.Size.Z - 2 * WELL_MARGIN)
    local originX = roomModel:GetAttribute("OriginX") or 0
    local originZ = roomModel:GetAttribute("OriginZ") or 0

    local well = self:_MakePart("LumenWell", Vector3.new(1.4, 1.4, 1.4), Vector3.new(originX + x, 1.2, originZ + z), WELL_COLOR, Enum.Material.Neon)
    well.Anchored = true
    well.CanCollide = false -- touch trigger (same pattern as artifact nodes)
    well:SetAttribute("Well", true)
    well:SetAttribute("Floor", floor)

    local core = Instance.new("PointLight")
    core.Name = "CoreLight" -- LumenService:DepleteWell dims this on use
    core.Color = WELL_COLOR
    core.Brightness = 1.5
    core.Range = 16
    core.Shadows = false
    core.Parent = well

    well.Parent = roomModel -- destroyed with the floor rebuild (fresh Well each floor)

    if self.LumenService ~= nil then
        self.LumenService:RegisterWell(well)
    end
    print(("[FloorService] Floor %d: Lumen Well placed in %s."):format(floor, template.Name))
end
